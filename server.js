...
function startServer() {
  ...
  class ChunkedUploadManager {
    ...
    addChunk(chunkIndex, chunkBuffer) {
      if (!this.chunks.has(chunkIndex)) {
        ...
        this.uploadedChunks++;
        this.uploadedBytes += chunkBuffer.length;
    ...
    broadcastProgress() {
      ...
      try {
        realTimeBroadcaster.broadcast(this.roomName, "chunk-assembly-progress", {
          ...
          elapsed: elapsed,
          userId: this.userId,
          message: `Assembling chunks: ${this.uploadedChunks}/${this.totalChunks}`
        });
      } catch (error) {
    ...
    async assembleFile() {
      try {
        ...
        console.log(`[ChunkedUpload] File assembled: ${this.fileName} (${this.fileSize} bytes)`);
  ...
  app.post('/upload-chunk', upload.single('chunk'), async (req, res) => {
    try {
      const { fileId, chunkIndex, totalChunks, fileName, fileSize, userId, roomName } = req.body;
      ...
      uploadManager.addChunk(parseInt(chunkIndex), chunkBuffer);

      console.log(`[ChunkedUpload] Chunk ${chunkIndex}/${totalChunks - 1} received for ${fileName}`);

      // Emit byte-progress event for real-time upload tracking
      const progress = (uploadManager.uploadedBytes / uploadManager.fileSize) * 100;
      const progressData = {
        fileId,
        ...
        totalBytes: uploadManager.fileSize,
        progress: Math.min(progress, 100),
        userId,
        timestamp: Date.now(),
        speed: uploadManager.uploadedBytes / ((Date.now() - uploadManager.startTime) / 1000) // bytes/sec
      };

      // Broadcast byte-progress event via SSE
      try {
        realTimeBroadcaster.broadcast(roomName, "byte-progress", progressData);
        console.log(`[ChunkedUpload] Emitted byte-progress for ${fileName}: ${progress.toFixed(1)}% (${uploadManager.uploadedBytes} bytes)`);
      } catch (error) {
        console.warn('[ChunkedUpload] Failed to emit byte-progress:', error.message);
      }
      ...
      res.json({
        ...
        uploadedChunks: uploadManager.uploadedChunks,
        totalChunks: uploadManager.totalChunks,
        isComplete: uploadManager.isComplete(),
        progress: progress,
        uploadedBytes: uploadManager.uploadedBytes
      });
